 ...
 * @param   lang      0(English)|1(Chinese)  Default Language is 0(English).
 */
function setmonth(callback,inputObj,dateFormatStyle,beginDate,endDate,lang){
    if(null == inputObj){return null;}
    new Calendar(inputObj,dateFormatStyle,beginDate,endDate,lang,"m").show(callback);
}
...
Calendar.prototype.valueSelected = function(date){
    ...
    if(this.type=='m'){
        var dates= date.format(this.dateFormatStyle);
        this.callback(dates);
    }
}
...
//鏄剧ず鏃ュ巻
Calendar.prototype.show = function (callback) {
